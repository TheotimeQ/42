...
{
	if (!form.Get_Sign_Status())
		std::cout << _Name << " couldn’t exec " << form.name() << " because it is not signed" << std::endl;
	else if (form.Get_Exec_Garde() < _Grade)
		std::cout << _Name << " couldn’t exec " << form.name() << " because of insufficient grade" << std::endl;
	else
	{
		try
		{
